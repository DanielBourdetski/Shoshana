...
export async function getAdminByUsername(username: string) : ResolvedPromise<Option<AdminUser>> {
    let res = await (client.db(dbName).collection(collectionAdmins).findOne({username}) as Promise<AdminUser | null>);
    return res ? some(res) : none(undefined);
}

export async function registerAdmin(username: string, password : string) : ResolvedPromise<Option<ObjectId>>{
    ...
    // inserting new user
    let res = await client.db(dbName).collection(collectionAdmins).insertOne({username, password});
